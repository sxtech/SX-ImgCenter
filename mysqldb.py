...
class ImgMysql:
    ...
    def getImgInfoByIPList(self,limit=10,ip=[]):
        try:
            strip = "','".join(ip)
            self.cur.execute("select i.id,i.pcip,i.inifile,i.imgpath,i.platecode,i.platecolor,i.roadname,i.directionid,i.channelid,i.pcip,d.d_ip,d.disk from indexcenter as i LEFT JOIN disk as d on i.disk_id = d.id where i.imgflag = 0 and i.pcip in('%s') and i.passdatetime>='%s' order by i.passdatetime desc limit 0,%s"%(strip,self.imgtime,limit))
            s = self.cur.fetchall()
        except MySQLdb.Error,e:
            raise
        else:
